...
public sealed class SpecimenTrackerEffectDispatcher : IMviEffectDispatcher<SpecimenTrackerEffect>
{
    ...
    public async ValueTask DispatchAsync(SpecimenTrackerEffect effect, CancellationToken cancellationToken = default)
    {
        ...
        {
            await _mediator.SendComponentInteractionAsync("检验医嘱", "标本流转 MVI", DashboardComponentActionKeys.Primary, primaryWorkflow.ContextText,
        ...
        {
            await _mediator.SendComponentInteractionAsync("检验医嘱", "标本流转 MVI", DashboardComponentActionKeys.Secondary, secondaryWorkflow.ContextText,
                cancellationToken).ConfigureAwait(false);
        }
    }
}
